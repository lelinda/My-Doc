      ...
      setTimeout(function () {
        doctorService.getDoctorsApi(name, specialty, coords)
          .then(function (response) {
            ...
            for (var i = 0; i < response.data.data.length; i++) {
              var contentString = '<div id="content">' + '<div id="siteNotice">' + '</div>' + '<h1 id="firstHeading" class="firstHeading">' + response.data.data[i].profile.first_name + ' ' + response.data.data[i].profile.middle_name + ' ' + response.data.data[i].profile.last_name + ', ' + response.data.data[i].profile.title + '</h1>' + '<p class="specialtyName">' + response.data.data[i].specialties[0].name + '</p>' + '<div id="bodyContent" class="doctorDescription">' + '<p>' + response.data.data[i].profile.bio + '</p>' + '<a ui-sref="doctor" class="info">+ more info</a>' + '</div>' + '</div>';
